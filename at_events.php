<?php include "includes/header.php" ?>
  ...
  <?php include "includes/topbar.php" ?>
    ...
    <?php
	 ...
	 if(isset($_GET['post'])){
		   $conn = $pdo->open();
		   $stmt = $conn->prepare("SELECT *,COUNT(*) AS numrows FROM events WHERE id=:id");
		   ...
		   //for comments and from events page
    ...
    <section class="ftco-section ftco-no-pt ftc-no-pb">
			<div class="container">
	          ...
	          $stmt->execute(['id'=>$events['name']]);
	         $person = $stmt->fetch();
			  ?>
	          	<h2 class="mb-4"><?php echo $events['head']; ?></h2>
                  <p>
                  <div class="col-lg-6">
<img src="admin/includes/gallery/<?php echo $events['photo']; ?>" width="100%" />
</div>
                  </p>
...
<div class="col-md-12">
 ...
 <input type="hidden" name="title" value="<?php echo $events['id']; ?>" />
...
$stmt->execute(['title'=>$events['id'], 'page'=>'events']);
foreach($stmt as $row){
$stmt = $conn->prepare("SELECT * FROM users WHERE id=:id");
...
$delete = '
<form method="post" action="comment_delete_events.php">
<input type="hidden" name="id" value="'.$row['id'].'" />
<input type="hidden" name="blog" value="'.$events['id'].'" />
...
<div class="comment-thread">
    ...
    <details open class="comment" id="comment-'.$row['id'].'">
		...
		<b style="text-decoration:underline; text-transform:uppercase;">'.$row['head'].'  </b>
            ...
            '.$rep1.'
            <!-- Reply form start -->
            <form method="POST" action="reply_events.php" class="reply-form d-none" id="comment-'.$row['id'].'-reply-form">
			   '.$input_1.'
...
$delet = '
<form method="post" action="reply_delete_events.php">
